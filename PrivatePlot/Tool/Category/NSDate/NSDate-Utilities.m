...
- (int)getYear{
    ...
    int year = [comps year];// 102  1102 112 1112
    return year;
}
...
- (int)getMonth{
    ...
    //int week=0;
    NSDateComponents *comps = [calendar components:unitFlags fromDate:self];
    int month = [comps month];// 102  1102 112 1112
    return month;
}
...
- (NSArray *)intervalSinceNow
{
    ...
    if (cha < 0) {
        cha = 0 - cha;
        return [[[self calcTheTime:YES withInterval:cha] retain] autorelease];
    }else{
        return [[[self calcTheTime:NO withInterval:cha] retain] autorelease];
    }
}
...
- (NSString *)daysSinceNow{
    ...
    if (cha < 0) {
        cha = 0 - cha;
        return [[[self calcTheDay:YES withInterval:cha] retain] autorelease];
    }else{
        return [[[self calcTheDay:NO withInterval:cha] retain] autorelease];
    }
}
